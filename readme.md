...
另外四个指尖。每项使用独立的固定旋转、scaling 和 weight；旋转把人手中立
向量直接对齐到 MMHand 的机械中位向量，因此 A-A 不需要额外补偿损失。机械
中位为小指、无名指、中指、食指、拇指 A-A 分别 36°、29°、31°、23°、28°，
也是 IK 初值。向量统一采用 SmoothL1，拇指 MCP/IP 另以相对中立姿态的几何
弯曲角为目标。上一帧原始角度既作 warm start，也用于小权重时序正则。没有
接触状态、动态增益、指腹方向、碰撞项、DexPilot、分段、阻尼最小二乘、Torch
或额外进程。运行时低通 `alpha=0.4`，所有映射和参数均在 `config.py`。
