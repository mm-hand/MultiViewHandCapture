"""Unified HandFrame source backed by the official MANUS Core 3.1.1 SDK."""
...
def _sdk_frame_to_packet(frame, received_at):
    ...
    return {
        ...
        "positions": positions,
        # Still the unmodified C-struct field order here. The adapter performs
        # the one and only convention conversion at the input boundary.
        "rotations": rotations,
        "node_ids": node_ids,
        "node_info": node_info,
...
class OfficialSdkTransport:
    ...
    def __init__(self, bridge_path=MANUS_SDK_BRIDGE_PATH, clock=time.monotonic):
        ...
        if not path.is_file():
            raise RuntimeError(
                f"MANUS SDK bridge is missing: {path}; run `make -C manus/assets`"
            )
        ...
        packets.append(
            {
                "glove_id": glove_id,
                "positions": values[:, :3],
                # Existing sender explicitly serializes rotation.x/y/z/w.
                "rotations": values[:, 3:7],
...
class ManusSource:
    """Read fresh WORLD Raw Skeleton frames and return normalized HandFrames."""
    ...
    def __init__(
        ...
        *,
        ...
        print("MANUS quaternion raw convention: ManusQuaternion fields w,x,y,z")
        print("HandFrame quaternion convention: xyzw")
        print("Thumb Tip orientation semantic: R_MANUS_WORLD_FROM_THUMB_TIP_NODE")
    ...
    def read(self):
        ...
        try:
            adapted = adapt_raw_skeleton(
                ...
                quaternion_convention=packet.get("quaternion_convention", "xyzw"),
            )
        ...
        if not self._first_frame_reported:
            ...
            print(f"MANUS Thumb Tip row: {adapted.thumb_tip_row}")
        ...
        return HandFrame(
            ...
            ready=ready,
            status=status,
            thumb_tip_orientation_world_xyzw=adapted.thumb_tip_orientation_world_xyzw,
            preview=None,
        )
